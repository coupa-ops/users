...
def customelogger(is_user_exists_on_system, is_user_session_active, custom_action, username, expiration_date)
  ...
  if is_user_exists_on_system
    ...
    elsif custom_action.to_s.eql?('remove')
  ...
  elsif custom_action.to_s.eql?('create')
...
action :create do
  ...
  if Chef::Config[:solo] and not chef_solo_search_installed?
    ...
    search(new_resource.data_bag, "groups:#{new_resource.search_group} AND NOT action:remove") do |u|
      ...
      if coupa_pay? && !whitelist_groups.include?(new_resource.group_name)
